...
%--#] The preprocessor variables : 
%--#[ List support and string manipulation :

\section{List support and string manipulation}
\label{listmanipulation}

\noindent Preprocessor variables can be treated as lists of comma-separated items. 
Various operations are available for extracting elements, slicing, determining 
the length, and searching for items. Additionally, a string replacement feature 
is available. These operations are invoked by appending square brackets with 
specific syntax to the preprocessor variable name.

\begin{description}
\item[{var[n]}] Returns the n-th element of the list. Indexing starts at 1. If 
the index is greater than the number of elements, an empty string is 
returned.
\item[{var[0]}] Returns the total number of elements in the list.
\item[{var[n:m]}] Returns a sublist starting from the n-th element up to and 
including the m-th element. If n is greater than m, an empty string is 
returned. 
If m is omitted (as in [n:]), all elements from n to the end are returned. 
If n is omitted (as in [:m]), elements from the start up to m are returned.
\item[{var[?string]}] Returns the 1-based index of the first occurrence of 
`string' in the list. If the string is not found, 0 is returned.
\item[{var[s/old/new/]}] Performs a string replacement, replacing all 
occurrences of `old' with `new'. Any character can be used as a delimiter if it 
is not present in `old' or `new' (e.g., [s!old!new!]).
\end{description}

\noindent All list operations are bracket-aware. This means that commas inside 
matched pairs of parentheses \verb:():, square brackets \verb:[]:, or curly 
brackets \verb:{}: are not treated as element separators. This allows 
elements to be function calls or other nested structures.

Example:
\begin{verbatim}
    #define mylist "f(a,b), [1,2], c"
    Local F1 = `mylist[1]';
    Local F2 = `mylist[2]';
    Local L  = `mylist[0]';
\end{verbatim}
...
add to it. If the path to the file contains directories that do not exist, 
...
file if it had not been opened yet at the moment of writing. If the path to 
the file contains directories that do not exist, they will be created 
automatically.
...
\noindent See also globaldefine (\ref{preglobaldefine}), 
           redefine (\ref{preredefine}), 
           undefine (\ref{preundefine})
...
\noindent See also enddo (\ref{preenddo}), sum (\ref{presum}), prod (\ref{preprod})
...
do\index{do loop} loop. See the \#do\index{\#do} instruction. The \#enddo 
instruction must match a \#do instruction; it cannot be used to close a
\#sum or \#prod loop.
...
\noindent See also prod (\ref{preprod})

\noindent Used to\index{\#endprod} terminate\index{terminate} a preprocessor 
prod\index{prod loop} loop. See the \#prod\index{\#prod} instruction. The 
\#endprod instruction must match a \#prod instruction; it cannot be used to 
close a \#do or \#sum loop.
...
\noindent See also sum (\ref{presum})

\noindent Used to\index{\#endsum} terminate\index{terminate} a preprocessor 
sum\index{sum loop} loop. See the \#sum\index{\#sum} instruction. The 
\#endsum instruction must match a \#sum instruction; it cannot be used to 
close a \#do or \#prod loop.
...
\#prod lvar = $\{$string1,...,stringn$\}$
 
\noindent See also endprod (\ref{preendprod}), do (\ref{predo}), 
                    sum (\ref{presum})

\noindent The \#prod\index{\#prod} instruction\index{prod loop} is a 
variant of the \#do\index{\#do} loop that is designed for constructing 
products. It needs a matching \#endprod\index{\#endprod} instruction. 
The syntax of the loop parameters (numerical ranges and listed loops) is 
the same as for the \#do instruction.

The key difference from a regular \#do loop is that the output of each 
iteration is wrapped in parentheses and the iterations are separated by 
multiplication signs. Hence the result is a product of terms rather than 
a simple concatenation.

If the loop range is empty (e.g.\ the end value is less than the start 
value), the \#prod loop produces the text \verb:1:, which is the 
...
\begin{verbatim}
   ...
   F =
      6 + 11*x + 6*x^2 + x^3;
\end{verbatim}
Here the \#prod loop produces \verb:(1+x)*(2+x)*(3+x): which FORM then 
expands into the polynomial $6 + 11x + 6x^2 + x^3$.

\#prod loops can be nested with each other and with \#do and \#sum loops, 
...
\section{\#globaldefine}
\label{preglobaldefine}
...
\noindent The \#globaldefine\index{\#globaldefine} instruction creates a new 
instance of a preprocessor\index{preprocessor variable} 
variable\index{variable!preprocessor} that persists even if it is defined 
inside a \#procedure or other nested scope. Unlike \#define, which creates 
a local instance that is automatically undefined when the current stream 
(like a procedure) is closed, \#globaldefine ensures that the definition 
is preserved and "bubbles up" to the global scope.
 
If a variable with the same name already exists in the current local scope, 
\#globaldefine will create a new entry (or update an existing global one) 
that will become visible once the local definitions are popped. This is 
particularly useful for procedures that need to return information to the 
global scope without requiring the variables to be pre-declared for use 
with \#redefine.
...
\#sum lvar = $\{$string1,...,stringn$\}$
 
\noindent See also endsum (\ref{preendsum}), do (\ref{predo}), 
                    prod (\ref{preprod})

\noindent The \#sum\index{\#sum} instruction\index{sum loop} is a variant 
of the \#do\index{\#do} loop that is designed for constructing sums. It 
needs a matching \#endsum\index{\#endsum} instruction. The syntax of the 
loop parameters (numerical ranges and listed loops) is the same as for 
the \#do instruction.

The key difference from a regular \#do loop is that a \verb:+: sign is 
automatically inserted between each iteration. Hence the result is a sum 
of terms rather than a simple concatenation.

If the loop range is empty (e.g.\ the end value is less than the start 
value), the \#sum loop produces the text \verb:0:, which is the identity 
...
\begin{verbatim}
   ...
   F =
      x + 2*x^2 + 3*x^3 + 4*x^4;
\end{verbatim}
Here the \#sum loop produces \verb:1*x^1+2*x^2+3*x^3+4*x^4: which FORM 
then evaluates.

\#sum loops can be nested with each other and with \#do and \#prod loops, 
following the usual nesting rules. A common pattern is to use \#sum 
together with \#prod:
\begin{verbatim}
    S x;
    Local F =
...
instruction should be used first. If the path to the file contains 
directories that do not exist, they will be created automatically.
...
\#sum\index{\#sum}/\#endsum\index{\#endsum}, 
\#prod\index{\#prod}/\#endprod\index{\#endprod}, 
